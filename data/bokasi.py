...
if __name__ == '__main__':
    ...
    for i in range(image_count):
        if os.path.isfile(input_data_path  +'\image(' + str(i) +')'+ '.jpeg'):
                img_src = cv2.imread(input_data_path  + '\image(' + str(i) + ')'+ '.jpeg')
                trans_img = []
                ...
                # 保存
                if not os.path.exists("mika_data"):
                    os.mkdir("mika_data")
                ...
                for i, img in enumerate(trans_img):
                    ...
                    cv2.imwrite("trans_images/" + base + str(i) + str(count) +".jpg" ,img)
                count = count +1
        else:
